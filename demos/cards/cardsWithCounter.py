...
class Game(simpleGE.Scene):
    def __init__(self):
        ...
        self.lblCounter = simpleGE.Label()
        self.lblCounter.text = f"Cards left: {self.cardsLeft}"
        self.lblCounter.position = (320, 400)
    ...
    def processCard(self, cardNum):
        "when a card is clicked, change its status"
        self.cards[cardNum].state = DISCARDED
        self.cardsLeft -= 1
        self.lblCounter.text = f"Cards left: {self.cardsLeft}"
    ...
    def process(self):
        ...
        if self.clicked:
            ...
            while keepGoing:
                ...
                if self.scene.cards[cardNum].state == DECK:
                    currentCard = self.scene.cards[cardNum]
                    self.copyImage(currentCard.image)
                    self.scene.processCard(cardNum)
                    keepGoing = False        
class Card(object):
